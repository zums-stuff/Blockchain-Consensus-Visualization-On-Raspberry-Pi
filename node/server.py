...
def start_miner(mining_option):
    ...
    try:
        ...
        # Iniciar cronómetro de tiempo general
        general_start_time = time.time()
        print(f"General start time recorded: {general_start_time}")

        # Iniciar generador de transacciones
        tx_process = subprocess.Popen(["python3", "tx_generator.py"])
        print("Tx Generator started")
        ...
        while True:
            ...
            # Flush CodeCarbon tracker periodically to update emissions.csv in real-time
            ...
            if current_block >= number_of_blocks_per_run:
                ...
                general_completion_time = general_end_time - general_start_time
                print(f"Target reached! Block {current_block} >= {number_of_blocks_per_run}")
                print(f"=== TIEMPO DE FINALIZACIÓN GLOBAL: {general_completion_time:.2f} segundos ===")
                
                # Guardar el tiempo global en un archivo
                try:
                    with open("general_completion_time.txt", "w") as f:
                        f.write(f"Start Time: {general_start_time}\n")
                        f.write(f"End Time: {general_end_time}\n")
                        f.write(f"Total Completion Time (seconds): {general_completion_time:.2f}\n")
                except Exception as e:
                    print(f"Error saving general time: {e}")
        ...
        # Detener visualización y generador
        print("Stopping visualization and Tx Generator...")
        process.terminate()
        try:
            tx_process.terminate()
        except:
            pass
        print("Visualization and Tx Generator stopped")

        # Detener tracker de CodeCarbon AQUI para que se cree el CSV
        # antes de intentar leerlo en save_block_data
        if tracker is not None:
            try:
                tracker.stop()
                tracker = None
                print("CodeCarbon tracker stopped successfully.")
            except Exception as e:
...
def start_mining_visualization():
    ...
    # Iniciar tracker de energía
    if tracker is None:
        tracker = OfflineEmissionsTracker(country_iso_code="CHL", output_dir=".")
    try:
        tracker.start()
    except Exception as e:
        print(f"Error starting CodeCarbon: {e}")
...
def stop_mining():
    ...
    try:
        ...
        # Detener tracker de energía para que escriba en emissions.csv
        if tracker is not None:
            try:
                tracker.stop()
            except Exception as e:
