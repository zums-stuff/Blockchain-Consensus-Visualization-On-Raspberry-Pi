...
def get_ip_dict(hostnames_list, username, password):
    ...
    for hostname in hostnames_list:
        #print("HoOSTNAME: ", hostname)
        ssh_client = connect_to_pi(hostname, username, password,ip_address_file)
        time.sleep(2)
        results = execute_commands_on_pi(ssh_client, commands)
        #print("RESULTS IP: ", results)
        ...
        for ip_item in results2:
            #print("IP ITEM: ",ip_item)
            first_three_numbers_ip=int(ip_item[0:3])
            #print("first_three_numbers_ip: ",first_three_numbers_ip)
            if first_three_numbers_ip==192:
                ip_address=ip_item
                #print("THIS IS THE IP: ",ip_address)
                break
            else:
                ip_address = results[0] if results else 'N/A'
...
def copy_file_to_raspberry_pis(hostnames, password, local_file_path, remote_file_path):
    try:
        for hostname in hostnames:
            ...
            ssh_client.connect(hostname, username="raspberry-node", password=password)
            time.sleep(1)

            #Use SCP to copy the pc network_stuc.json file to the Pis
            ...
            print(f"File copied to {hostname} successfully.")
